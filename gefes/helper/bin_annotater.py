...
class BinAnnotater(object):
    ...
    def single_copy_cog_blast(self):
        ...
        output = []
        single = []
        weird = [] # count of perfectly similar high quality hits
        idx = -1
        for caller in self.bini.caller:
            ...
            for db in scc_dbs:
                    ...
                    if os.stat(b_query.out_path).st_size > 0:
                            ...
                            processed = pandas.DataFrame(columns=raw.columns)
                            for orf in orfs:
                                    max_es = raw.loc[orf][10] == raw.loc[orf][10].min()
                                    if isinstance(max_es, pandas.Series):
                            ...
                            if len(output[idx][db[0]]) == 0 : single[idx][db[0]] = 'abscent'
                                ...
                                coords = numpy.array([sorted(list(z)[1:]) for z in data.itertuples()])
                                rnk = numpy.argsort([z[0] for z in coords])
                    ...
                    pandas.DataFrame(single[idx],index=["presence"]).transpose().to_csv(path + "single_copy_cogs.tsv",sep="\t")
                    pandas.DataFrame(weird[idx],index=["presence"]).transpose().to_csv(path + "auto_chimeras.tsv",sep="\t")
        self.single = single
        self.weird = weird